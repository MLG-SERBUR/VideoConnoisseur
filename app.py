...
def scan_videos():
    """Scan the video directory and add new videos to the database."""
            ...
            for file in files:
                if any(file.lower().endswith(ext) for ext in allowed_extensions):
                    ...
                    if not cursor.fetchone():
                        ...
                        conn.execute(
                            """
                            INSERT INTO videos (file_path, file_name, file_size)
                            VALUES (?, ?, ?)
                        """,
                        ...
                        videos_found += 1
        conn.commit()
        logger.info(f"Scan complete: found {videos_found} new videos")
...
def get_or_create_weights():
    ...
    for feature, default_weight in default_weights.items():
        if feature not in weights:
            ...
            conn = get_db_connection()
            conn.execute(
                "INSERT INTO weights (feature_name, weight) VALUES (?, ?)",
    ...
    for feature_name, feature_value in video_features.items():
        ...
        if contribution != 0:
            score_details.append(
                f"{feature_name}: {feature_value} * {weight} = {contribution}"
            )
...
app = Flask(
    __name__, static_folder="templates", static_url_path="", template_folder="templates"
)
...
def serve_video(filename):
    ...
    videos_dir_norm = videos_dir.replace(os.sep, "/")
    filename_norm = filename.replace(os.sep, "/")
    if filename_norm.startswith(videos_dir_norm):
        relative_path = filename_norm[len(videos_dir_norm) :].lstrip("/")
        full_path = os.path.join(videos_dir, relative_path.replace("/", os.sep))
...
def vote_video(video_id):
    ...
    visual_features = conn.execute(
        "SELECT * FROM visual_features WHERE video_id = ?", (video_id,)
    ).fetchone()
...
if __name__ == "__main__":
    ...
    # Scan for new videos
    # We run this always, but if the reloader is active,
    # we want to ensure it actually prints to the console.
    scan_videos()
